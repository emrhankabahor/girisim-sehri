(async function(){
  ...
  try{
    ...
    const parts=await Promise.all(files.map(f=>fetch(f+'?v=170',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(f+' '+r.status);return r.text()})));
    ...
    script.onload=()=>{
      ...
      patch.src='v167.js?v=170';
      patch.onload=()=>{
        const realtime=document.createElement('script');
        realtime.src='realtime-finance.js?v=170';
