/* Girişim Şehri • Birleşik kayıt ve veri bütünlüğü katmanı */
(function(){
  const SNAPSHOT_KEY='gs_state_integrity_snapshot_v1';
  const SNAPSHOT_PREV_KEY='gs_state_integrity_snapshot_prev_v1';
  const VERSION=1;
  ...
  function normalizeStateInPlace(){
    try{
      ...
      if(typeof cash!=='undefined')cash=Math.max(0,num(cash,0));
      ...
      if(typeof tx!=='undefined')tx=arr(tx).filter(x=>x&&typeof x==='object').slice(0,500);
      if(typeof loans!=='undefined'){
        loans=arr(loans).filter(x=>x&&typeof x==='object').map(l=>({
          ...l,amount:Math.max(0,num(l.amount)),remaining:Math.max(0,num(l.remaining)),
      ...
      sim.monthlyHistory=arr(sim.monthlyHistory);sim.currentMonth=obj(sim.currentMonth);
      sim.companyProfile=obj(sim.companyProfile);

      const seen=new Set();
      sim.companies=sim.companies.map(normalizeCompany).filter(c=>c&&c.name&&!seen.has(c.id)&&(seen.add(c.id),true));
  ...
  function persistSnapshot(){
    try{
      ...
      const state=capture();if(!state)return;
      ...
      const uid=currentUserId();if(uid)localStorage.setItem('gs_account_career_'+uid,JSON.stringify(state));
    }catch(e){console.warn('State snapshot:',e)}
  }
  ...
  function companyCandidatesFromStorage(){
    ...
    try{takeState(safeParse(localStorage.getItem(SNAPSHOT_PREV_KEY)))}catch(e){}
  ...
  function repairCompaniesFromStorage(){
    try{
      ...
      const byId=new Map(sim.companies.map(c=>[c.id,c]));
      companyCandidatesFromStorage().forEach(c=>{if(!byId.has(c.id))byId.set(c.id,c)});
      ...
      if(!sim.selectedCompanyId||!merged.some(c=>c.id===sim.selectedCompanyId))sim.selectedCompanyId=merged[0].id;
      const selected=merged.find(c=>c.id===sim.selectedCompanyId)||merged[0];
      sim.companyProfile={...sim.companyProfile,...selected,established:true};sim.companyName=selected.name;
      persistSnapshot();
      return true;
  ...
  function wrapPersistence(){
    try{
      if(typeof simSave==='function'&&!simSave.__integrityWrapped){const orig=simSave;const wrapped=function(){const r=orig.apply(this,arguments);setTimeout(persistSnapshot,0);return r};wrapped.__integrityWrapped=true;simSave=wrapped}
    }catch(e){}
    try{
      if(typeof saveOwned==='function'&&!saveOwned.__integrityWrapped){const orig=saveOwned;const wrapped=function(){const r=orig.apply(this,arguments);setTimeout(persistSnapshot,0);return r};wrapped.__integrityWrapped=true;saveOwned=wrapped}
    }catch(e){}
  }

  function refresh(){
    normalizeStateInPlace();removeLegacyMonthArtifacts();repairCompaniesFromStorage();persistSnapshot();
    try{if(typeof renderCompanyPortfolio==='function')renderCompanyPortfolio()}catch(e){}
  }

  wrapPersistence();
  setTimeout(refresh,120);setTimeout(refresh,800);setTimeout(refresh,2200);
  ...
  setInterval(persistSnapshot,30000);
})();
