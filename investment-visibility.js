...
(function(){
  ...
  /* Performans: yalnızca bootstrap içindeki iki ağır arka plan zamanlayıcısını seyrekleştirir.
     Menü/hash/geçiş kodlarına dokunmaz; diğer setInterval çağrılarını aynen bırakır. */
  if(!window.__eotSafeIntervalTuning){
    window.__eotSafeIntervalTuning=true;
    var nativeSetInterval=window.setInterval.bind(window);
    window.setInterval=function(fn,delay){
      var name=fn&&fn.name?fn.name:'';
      var nextDelay=delay;
      if(name==='syncDemo' && delay===600) nextDelay=1800;
      else if(name==='checkRemoteVersion' && delay===30000) nextDelay=120000;
      var args=Array.prototype.slice.call(arguments,2);
      return nativeSetInterval.apply(window,[fn,nextDelay].concat(args));
    };
  }
  ...
  function addStyle(){if(document.getElementById('eot-trade-total-safe-style'))return;var st=document.createElement('style');st.id='eot-trade-total-safe-style';st.textContent='\
...
';document.head.appendChild(st);}
  ...
  var tries=0;
  var bootTimer=setInterval(function(){tries++;install();if(tries>=12)clearInterval(bootTimer)},650);
  setInterval(function(){if(document.hidden)return;document.querySelectorAll('input[id^="tradeqty_"][data-eot-total-ready="1"]').forEach(updateTotal);document.querySelectorAll('.asset-card input[id^="qty_"][data-eot-card-total-ready="1"]').forEach(updateCardTotal);},6000);
})();
