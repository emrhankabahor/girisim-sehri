...
(function(){
  ...
  function prepareMoneyInputs(root=document){root.querySelectorAll('input').forEach(el=>{if(!isMoneyInput(el)||el.dataset.tryFormat==='1')return;el.dataset.tryFormat='1';el.type='text';el.inputMode='decimal';el.addEventListener('focus',()=>{if(el.value)el.value=formatTRYInput(el.value)});el.addEventListener('input',()=>{let caret=el.selectionStart||0,before=el.value.length;el.value=el.value.replace(/[^0-9,.]/g,'');let after=el.value.length;try{el.setSelectionRange(Math.max(0,caret-(before-after)),Math.max(0,caret-(before-after)))}catch(e){}});el.addEventListener('blur',()=>{if(el.value)el.value=formatTRYInput(el.value)});if(el.value)el.value=formatTRYInput(el.value)})}
  ...
  window.acceptLoan=function(id,name,limit,rate,maxTerm){if(blockThirdLoan())return false;let requested=parseTRY(document.getElementById('loanamt_'+id)?.value),months=Number(document.getElementById('loanterm_'+id)?.value);if(!requested||requested<100000||requested>limit){if(typeof toast==='function')toast('Kredi tutarı geçersiz');return false}if(!months||months<1||months>maxTerm||months>6){if(typeof toast==='function')toast('Vade 1-6 ay olmalı');return false}let rawTrust=Number(trusts[id]),trust=Number.isFinite(rawTrust)?clamp(rawTrust,0,100):50,assess=creditAssessment(limit,trust),score=Number(assess.score||0),allowed=Math.min(limit,Math.max(100000,Number(assess.limit||0)));if(score<25){let reasons=[];if(Number(creditScore)<30)reasons.push('kredi puanı düşük');if(trust<30)reasons.push('banka güveni düşük');if(Number(assess.debtRatio||0)>.65)reasons.push('borç oranı yüksek');if(Number(assess.income||0)<=0)reasons.push('düzenli gelir görünmüyor');if(!reasons.length)reasons.push('genel finansal skor yetersiz');if(typeof toast==='function')toast('Başvuru reddedildi • '+reasons.join(' • '));return false}let amount=Math.min(requested,allowed);amount=Math.floor(amount/1000)*1000;if(amount<100000){if(typeof toast==='function')toast('Onaylanan kredi tutarı minimum kullanım tutarının altında');return false}let riskPremium=score<40?.9:score<55?.45:score>=80?-.3:0,r=Math.max(.5,rate+riskPremium+(trust>=80?-.2:trust<40?.35:0)+((sim.macro?.rate||42.5)-42.5)/25+currentNews().credit*2),total=amount*(1+(r/100)*months),inst=total/months,d=new Date();d.setMonth(d.getMonth()+1);cash+=amount;loans.push({id,name,amount,requestedAmount:requested,approvedLimit:allowed,assessmentScore:score,rate:r,months,total,installment:inst,remaining:total,t:Date.now(),nextDue:d.getTime(),paidCount:0,hadLate:false,closed:false});trusts[id]=clamp(trust+2,0,100);tx.unshift({t:Date.now(),kind:'loan',type:'loan_in',sym:name,total:amount});save();if(typeof saveAccountCareer==='function'){try{let u=currentAccount();if(u&&u.id&&u.id!=='guest')saveAccountCareer(u.id)}catch(e){}}render();renderFinanceExtras();renderGameExtras();let st=document.getElementById('loanstatus_'+id);if(st)st.innerHTML='✅ Kredi kullanıldı • Banka skoru: <b>'+Math.round(score)+'/100</b> • Onaylanan tutar: <b>'+money(amount)+'</b> • Aylık taksit: <b>'+money(inst)+'</b>';if(requested>amount){if(typeof toast==='function')toast('Talebin '+money(requested)+' • Banka '+money(amount)+' onayladı ve bakiyene aktardı')}else if(typeof toast==='function')toast(name+' kredisi hesabına geçti');return true};
