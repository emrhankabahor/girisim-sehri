...
(function(){
  ...
  function formatTRYInput(v){
    let n=parseTRY(v);if(!n)return '';
    return n.toLocaleString('tr-TR',{minimumFractionDigits:0,maximumFractionDigits:2});
  }
  function isMoneyInput(el){
    if(!el||el.tagName!=='INPUT')return false;
    let key=((el.id||'')+' '+(el.name||'')+' '+(el.placeholder||'')).toLowerCase();
    return /loanamt|creditamount|capital|sermaye|tutar|amount|price|fiyat|cash|nakit/.test(key)&&!/term|vade|month|ay/.test(key);
  }
  function prepareMoneyInputs(root=document){
    root.querySelectorAll('input').forEach(el=>{
      if(!isMoneyInput(el)||el.dataset.tryFormat==='1')return;
      el.dataset.tryFormat='1';el.type='text';el.inputMode='decimal';
      el.addEventListener('focus',()=>{if(el.value)el.value=formatTRYInput(el.value)});
      el.addEventListener('input',()=>{
        let caret=el.selectionStart||0,before=el.value.length;
        el.value=el.value.replace(/[^0-9,.]/g,'');
        let after=el.value.length;try{el.setSelectionRange(Math.max(0,caret-(before-after)),Math.max(0,caret-(before-after)))}catch(e){}
      });
      el.addEventListener('blur',()=>{if(el.value)el.value=formatTRYInput(el.value)});
      if(el.value)el.value=formatTRYInput(el.value);
    });
  }
  ...
  const originalBusinessCredit=window.takeBusinessCredit;
  if(typeof originalBusinessCredit==='function')window.takeBusinessCredit=function(){let el=document.getElementById('businessCreditAmount');if(el)el.value=String(parseTRY(el.value));return originalBusinessCredit.apply(this,arguments)};
  const originalSecuredLoan=window.takeSecuredLoan;
  if(typeof originalSecuredLoan==='function')window.takeSecuredLoan=function(){if(blockThirdLoan())return false;document.querySelectorAll('input').forEach(el=>{if(isMoneyInput(el)&&el.value)el.value=String(parseTRY(el.value))});return originalSecuredLoan.apply(this,arguments)};
  ...
  const oldRender=window.render;if(typeof oldRender==='function')window.render=function(){const r=oldRender.apply(this,arguments);setTimeout(()=>{addLoanRuleInfo();updateLoanButtons();prepareMoneyInputs()},0);return r};
  const observer=new MutationObserver(()=>prepareMoneyInputs());observer.observe(document.body,{childList:true,subtree:true});
  setTimeout(()=>{addLoanRuleInfo();updateLoanButtons();prepareMoneyInputs()},300);
})();
