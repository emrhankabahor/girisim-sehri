/* Empire of Trade • Alt menü sabitleme ve güvenli yönlendirme */
(function(){
  'use strict';
  ...
  function sectionOf(btn){
    ...
    return String(btn&&btn.getAttribute('href')||'').replace(/^#/,'');
  }
  ...
  function syncActive(){
    ...
    const current=(location.hash||'#home').slice(1).toLocaleLowerCase('tr-TR');let matched=false;
  ...
  /* Ana sekmelerde özel history/hash emülasyonu kullanmıyoruz. Normal anchor davranışı
     uygulamanın mevcut :target ve hashchange kodlarıyla uyumlu kalır. */
  document.addEventListener('click',function(e){
    const btn=e.target&&e.target.closest?e.target.closest('.bottom-nav .nav-btn'):null;if(!btn)return;
    const section=sectionOf(btn);if(!section)return;
    const target=document.getElementById(section);
    if(!target)return;
    e.preventDefault();
    if(location.hash==='#'+section){resetTop();syncActive();return;}
    location.hash=section;
  },true);

  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',lock,{once:true});else lock();
  window.addEventListener('hashchange',function(){resetTop();syncActive()});
