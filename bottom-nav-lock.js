/* Empire of Trade • Alt menü sabitleme + tek-yollu hızlı yönlendirme */
(function(){
  'use strict';
  ...
  function belongs(section,current){
    ...
    if(section==='business')return['compan','business','factory','construction','asset'].some(x=>current.includes(x));
  ...
  function setActiveSection(section){
    ...
    nav.querySelectorAll('.nav-btn').forEach(btn=>{
      const active=!matched&&sectionOf(btn)===section;
  ...
  function bindFastNavigation(){
    ...
    nav.addEventListener('click',function(e){
      const btn=e.target.closest('.nav-btn');
      if(!btn||!nav.contains(btn))return;
      const section=sectionOf(btn),target='#'+section,now=performance.now();
      e.preventDefault();
      e.stopPropagation();
  ...
  window.addEventListener('hashchange',function(){
    const current=(location.hash||'#home').slice(1).toLocaleLowerCase('tr-TR');
    if(pendingSection&&belongs(pendingSection,current))pendingSection='';
    syncActive();
  });
