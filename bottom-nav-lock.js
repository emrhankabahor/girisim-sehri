...
(function(){
  ...
  function ensureBusinessCreditLimitFix(){if(!window.__eotBusinessCreditLimitFix)loadOnce('eot-business-credit-limit-fix-loader','business-credit-limit-fix.js?v=3','high')}
  ...
  function lock(){
    ...
    ensureDirectRouteDisplay();ensureRouteScrollReset();ensureBusinessHierarchy();ensureBusinessCreditLimitFix();ensureTransitionPerformance();ensurePersistenceDedupe();ensureVisibleRouteGuard();
    queueNonCritical();
    const nav=document.querySelector('.bottom-nav');if(!nav)return;
