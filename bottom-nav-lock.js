...
(function(){
  ...
  function ensureStyle(){
    ...
    s.textContent=`
      ...
      body>.bottom-nav .nav-btn .nav-ico{width:28px!important;height:28px!important;display:grid!important;place-items:center!important;margin:0!important;font-size:22px!important;line-height:1!important;color:#9eb6cd!important;transition:color .12s ease,transform .12s ease!important}
      body>.bottom-nav .nav-btn.active{background:rgba(35,75,116,.52)!important;color:#f5fbff!important}
      body>.bottom-nav .nav-btn.active .nav-ico{color:#d9efff!important;transform:translateY(-1px)!important}
      body>.bottom-nav .nav-btn:active{transform:scale(.985)!important}
      ...
      @media(prefers-reduced-motion:reduce){body>.bottom-nav .nav-btn,body>.bottom-nav .nav-btn .nav-ico{transition:none!important}}
    `;
  ...
  function beginTransition(btn){
    ...
    setActiveButton(btn);
    transitionObserver=new MutationObserver(function(){
      if(applyingActive||!transitionBtn)return;
      if(performance.now()>=transitionUntil){finishTransition();return;}
      if(!transitionBtn.classList.contains('active'))setActiveButton(transitionBtn);
    });
    transitionObserver.observe(nav,{subtree:true,attributes:true,attributeFilter:['class']});
    transitionTimer=setTimeout(finishTransition,720);
  }
  function syncActive(){
    const nav=document.querySelector('.bottom-nav');if(!nav)return;
    if(transitionBtn&&performance.now()<transitionUntil){setActiveButton(transitionBtn);return;}
  ...
  function bindImmediateActive(){
    const nav=document.querySelector('.bottom-nav');if(!nav||nav.dataset.eotImmediateActive==='3')return;
    nav.dataset.eotImmediateActive='3';
    nav.addEventListener('pointerdown',function(e){
      const btn=e.target.closest('.nav-btn');
      if(btn&&nav.contains(btn))beginTransition(btn);
    },{capture:true,passive:true});
    nav.addEventListener('click',function(e){
      const btn=e.target.closest('.nav-btn');
      if(btn&&nav.contains(btn)&&btn!==transitionBtn)beginTransition(btn);
    },true);
  }
  function lock(){
    ensureStyle();
  ...
  window.addEventListener('hashchange',function(){
    const current=(location.hash||'#home').slice(1).toLocaleLowerCase('tr-TR');
    if(transitionBtn&&belongs(sectionOf(transitionBtn),current)){
      transitionUntil=performance.now()+140;
      if(transitionTimer)clearTimeout(transitionTimer);
      transitionTimer=setTimeout(finishTransition,160);
    }
    syncActive();
  });
